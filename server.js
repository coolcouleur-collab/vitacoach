...
webpush.setVapidDetails(
  process.env.VAPID_EMAIL || 'mailto:contact@reva.app',
...
app.get('/', (req, res) => res.json({ status: 'Reva OK' }))
...
app.post('/api/chat', async (req, res) => {
  ...
  const systemPrompt = `Tu es Reva, un coach de vie personnel et bienveillant.
...
app.post('/api/routine', async (req, res) => {
  ...
  const prompt = `Tu es Reva, coach de vie IA. Génère une routine de journée personnalisée pour ${profil.nom}.
...
app.post('/api/health-insights', async (req, res) => {
  ...
  const prompt = `Tu es Reva, coach santé. Analyse les métriques de ${profil.nom} et donne 3 insights personnalisés.
...
app.post('/api/herbal', async (req, res) => {
  ...
  const prompt = `Tu es Reva, expert en phytothérapie et médecine naturelle. Analyse le profil de ${profil.nom} et propose des plantes/remèdes VRAIMENT personnalisés.
...
app.listen(PORT, () => {
  console.log(`✅ Serveur Reva démarré sur port ${PORT}`)
})
