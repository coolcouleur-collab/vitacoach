...
function SolennFace({ size = 34 }) {
  return (
    <div style={{
      ...
      boxShadow: '0 4px 14px rgba(255,113,18,0.42), inset 0 1px 0 rgba(255,255,255,0.25)',
      marginTop: 4,
    }}>
      {/* Reflet */}
      <div style={{
  ...
  return (
    <div style={s.app}>
      ...
      {!isMobile && (
        <aside style={s.sidebar}>
          <div style={s.sidebarTop}>
            <ShinyLogoText
              text="Solenn"
              gradient="linear-gradient(90deg, #FF7112 0%, #FFD060 18%, #FFFDE0 34%, #FFD060 50%, #FF7112 66%, #FFD060 82%, #FF7112 100%)"
              animDuration="4s"
          ...
          <nav style={s.sidebarNav}>
            {navItems.map(({ id, Icon, label }) => {
              ...
              const color = active ? '#FF7112' : '#8a7265'
              return (
          ...
          <div style={s.sidebarBottom}>
            <div style={s.profileCard}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                {profil.objectifs?.[0] && <div style={s.profileMeta}><TargetIcon size={13} color="#FF7112" /> {profil.objectifs[0]}</div>}
              </div>
            </div>
            {!isPro && (
              ...
              <div style={{
                ...
                {navItems.map(({ id, Icon, label }) => (
                  <button key={id} onClick={() => { setOnglet(id); setMenuOpen(false) }} style={{
                    ...
                    border:'none', background: onglet===id ? 'rgba(255,113,18,0.08)' : 'transparent',
                    cursor:'pointer', fontFamily:F, width:'100%', textAlign:'left',
                    color: onglet===id ? '#FF7112' : '#4a3728', fontWeight: onglet===id ? 700 : 500,
                    fontSize:14, marginBottom:3,
                    boxShadow: onglet===id ? 'inset 0 0 0 1px rgba(255,113,18,0.18)' : 'none',
                  }}>
                    <Icon color={onglet===id ? '#FF7112' : '#8a7265'} size={18} />
                    {label}
                  </button>
                ))}
                {/* Bottom actions */}
          ...
          {onglet === 'chat' && (
            <div style={s.chatWrap}>
              ...
              <div style={isMobile ? s.tabHeaderMobile : s.pageHeader}>
                ...
                <div>
                  <div style={{...s.pageTitle, display:'flex', alignItems:'center', gap:8}}>{!isMobile && <ChatIcon size={20} color="#FF7112" />} Coach IA</div>
                      ...
                      <span style={{ display:'inline-flex', gap:5, alignItems:'center' }}>
                        {[0, 0.18, 0.36].map((d,i) => (
                          <span key={i} style={{ width:7, height:7, borderRadius:'50%',
                            background:'linear-gradient(135deg,#FF7112,#FFB060)',
          ...
          return (
            <nav style={{
              ...
              <div style={{
                ...
                background:'rgba(255,113,18,0.10)',
                borderRadius:100,
                border:'1px solid rgba(255,113,18,0.22)',
                boxShadow:'0 2px 10px rgba(255,113,18,0.16)',
                transition:'left 0.40s cubic-bezier(0.34,1.56,0.64,1)',
                pointerEvents:'none', zIndex:0,
              }} />
              {navItems.map(({ id, Icon, label }) => {
                const active = onglet === id
                const color  = active ? '#FF7112' : '#9e8c7c'
                return (
                  <button key={id}
                    style={{
      ...
      <style>{`
        ...
        @keyframes pulseDot2 {
          0%,100% { transform:scale(1);    opacity:0.85; box-shadow:0 0 8px rgba(255,113,18,0.4); }
          50%      { transform:scale(1.4);  opacity:1;    box-shadow:0 0 16px rgba(255,113,18,0.7); }
        }
        @keyframes pulseDot3 {
          0%,100% { transform:scale(1);    opacity:0.7; }
          50%      { transform:scale(1.35); opacity:1; }
        ...
        * { -webkit-tap-highlight-color: transparent; }
        ...
        ::-webkit-scrollbar-thumb {
          background:linear-gradient(180deg, #FF7112 0%, #FFB060 100%);
          border-radius:10px;
          transition:width 0.3s ease;
        }
        ::-webkit-scrollbar-thumb:hover { background:linear-gradient(180deg,#FFB060,#FF7112); }
        * { scrollbar-width:thin; scrollbar-color:#FF7112 transparent; }
      `}</style>
    </div>
  )
}
      ...
      {stepsTotal > 0 && (
        <div style={sr.progressBar}>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
            ...
            <span style={{ fontSize:12, color:'#FF7112', fontWeight:700 }}>{doneTotal}/{stepsTotal}</span>
          </div>
          <div style={{ height:6, background:'#f0e8e0', borderRadius:4, overflow:'hidden' }}>
            <div style={{ height:'100%', width:`${stepsTotal>0?(doneTotal/stepsTotal)*100:0}%`,
              background:'linear-gradient(90deg,#FF7112,#FFB060)', borderRadius:4, transition:'width 0.4s ease' }} />
          </div>
        </div>
      )}
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          {routine.motivation && (
            <div style={sr.motivCard}>
              <div style={{ marginBottom:8 }}><SparkleIcon size={20} color="#FF7112" /></div>
              <div style={{ fontSize:14, fontWeight:600, color:'#FF7112', lineHeight:1.65 }}>{routine.motivation}</div>
            </div>
          )}
          {routine.matin && (
            <RoutineSection id="matin" iconEl={<SunIcon size={18} color="#FF7112" />} titre={routine.matin.titre} heure={routine.matin.heure}
          ...
          {routine.astuce && (
            <div style={sr.card}>
              <div style={sr.cardHeader}>
                ...
                <span style={{ ...sr.cardTitre, color:'#FF7112' }}>{routine.astuce.titre}</span>
              </div>
...
const sr = {
  ...
  btnGen: { background:'linear-gradient(135deg,#FF7112,#d45a00)', color:'#fff', border:'none',
    padding:'10px 18px', borderRadius:13, fontSize:12, fontWeight:700, cursor:'pointer',
    boxShadow:'0 4px 16px rgba(255,113,18,0.38)', flexShrink:0, fontFamily:"'Inter',system-ui,sans-serif" },
  ...
  return (
    <div style={st.tenueCard}>
      ...
      <div style={st.tenueInfo}>
        ...
        <div style={{...st.tenuePourquoi, display:'flex', alignItems:'center', gap:5}}><LightbulbIcon size={13} color="#FFB060" /> {tenue.pourquoi}</div>
      </div>
    </div>
  )
}
...
const st = {
  ...
  panel: { background:'#ffffff', border:'1px solid #f0e8e0', borderRadius:16, padding:14, marginTop:4,
    boxShadow:'0 2px 12px rgba(0,0,0,0.05)' },
  meteoBar: { background:'rgba(255,113,18,0.06)', borderRadius:10, padding:'8px 14px',
    fontSize:12, marginBottom:12, color:'#FF7112', fontWeight:600, border:'1px solid rgba(255,113,18,0.16)' },
  ...
  btn: { padding:'10px 16px', background:'linear-gradient(135deg,#FF7112,#d45a00)', color:'#fff',
    border:'none', borderRadius:12, fontSize:16, fontWeight:700, cursor:'pointer',
    fontFamily:"'Inter',system-ui,sans-serif", boxShadow:'0 4px 12px rgba(255,113,18,0.35)' },
  ...
  tenueTitre: { fontWeight:700, color:'#FF7112', fontSize:13, marginBottom:5 },
...
const s = {
  ...
  logo: {
    ...
    background:'linear-gradient(90deg, #FF7112 0%, #FFD060 18%, #FFFDE0 34%, #FFD060 50%, #FF7112 66%, #FFD060 82%, #FF7112 100%)',
    backgroundSize:'250% 100%',
  ...
  navActive: {
    display:'flex', alignItems:'center', gap:10, padding:'11px 14px', borderRadius:14,
    border:'none', background:'rgba(255,113,18,0.07)',
    cursor:'pointer', fontFamily:F, color:'#FF7112', fontWeight:700,
    textAlign:'left', width:'100%', fontSize:13, transition:'all .2s',
    boxShadow:'inset 0 0 0 1.5px rgba(255,113,18,0.18)',
  },
  profileCard: {
  ...
  avatar: {
    ...
    boxShadow:'0 4px 12px rgba(255,113,18,.35)',
  },
  profileName: { fontSize:13, fontWeight:700, color:'#1a0a00', marginBottom:1 },
  profileMeta: { fontSize:10, color:'#c4b5a8', lineHeight:1.5 },
  btnPro: {
    background:'linear-gradient(135deg,#FF7112,#d45a00)', color:'#fff', border:'none',
    padding:'11px 14px', borderRadius:12, cursor:'pointer', fontSize:12, fontFamily:F,
    fontWeight:700, boxShadow:'0 6px 20px rgba(255,113,18,.35)', textAlign:'center',
  },
  proBadge: {
    background:'rgba(255,113,18,.08)', color:'#FF7112',
    border:'1px solid rgba(255,113,18,.22)',
  ...
  userBubble: {
    background:'linear-gradient(145deg,#FF7112,#d45a00)', color:'#fff',
    padding:'13px 18px', borderRadius:'20px 20px 5px 20px', maxWidth:'76%',
    fontSize:14, lineHeight:1.65,
    boxShadow:'0 8px 28px rgba(255,113,18,.38), inset 0 1px 0 rgba(255,255,255,.2)',
  },
  botBubble: {
  ...
  botAvatar: { fontSize:16, color:'#FF7112', marginTop:10, flexShrink:0, fontWeight:900 },

  suggestionsRow: { display:'flex', gap:7, marginBottom:10, flexWrap:'wrap' },
  suggestion: {
    background:'#ffffff', border:'1px solid rgba(255,113,18,.20)', borderRadius:20,
    padding:'7px 14px', fontSize:12, color:'#FF7112', cursor:'pointer',
  ...
  sendBtn: {
    background:'linear-gradient(135deg,#FF7112,#d45a00)', border:'none',
    width:42, height:42, borderRadius:14, cursor:'pointer',
    display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0,
    boxShadow:'0 4px 16px rgba(255,113,18,.40)',
  },
